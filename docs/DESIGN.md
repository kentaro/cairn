...
- **Claude Code が遅い真因は「素朴なローカルサーバ」と「CC の前提」のミスマッチ**。
  実 CC ターンは `cached=17/45040` ＝毎ターン byte-非同一のヘッドを送るため
...
- **ただし CC のプロンプト設計が悪いわけではない**。CC は Anthropic 本家バックエンドの
  **サーバ側コンテキスト最適化（明示 prompt caching）と一体で設計されている**。
  ...
  これが本家で CC が実用的に動く理由。**ローカルが遅いのは、(a) vllm-mlx が自動
  byte-prefix しか持たず Anthropic 式の明示キャッシュを honor しない（要検証）、
  (b) Apple silicon の prefill が遅い、の二重苦**であって、CC やサーバ単体の欠陥
  ではない。
...
- **要検証: vllm-mlx は Anthropic の `cache_control`（明示 prompt caching）を honor するか**。
  もし honor するなら、CC が動的ヘッダを末尾に置く設計のまま、安定ブロック（tools/system）
  だけターン跨ぎでキャッシュヒットでき、ローカル CC のレイテンシを大幅に救える可能性がある
  （§8 結論の (a) を解消できるか）。`cache_control` 付きリクエストを 2 回投げて
  `cached=` が伸びるかで判定する実験を追加する。
