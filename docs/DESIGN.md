...
- ~~Codex プロファイルは env のみで model_provider 設定が別途必要~~ → **解決済み**。
  `cairn codex` は `build_args` で `-c model_providers.cairn.{base_url,wire_api=responses,
  env_key}` ＋ `-c model_provider=cairn` ＋ `-m <model>` を自動注入するので、`~/.codex/
  config.toml` を編集せず一発でローカルに繋がる（実機で `model: <local>` 表示・
  `POST /v1/responses` がローカルサーバに着弾することを確認）。
